...
Page({
  ...
  addClasses: function () {
    if (this.data.userType != 'admin' && this.data.userType != 'comptAdmin') {
      wx.showToast({
        title: '无访问权限',
        image: '/images/about.png'
      })
    ...
    wx.navigateTo({
      url: '/pages/detail/detail?id=-1&db=compt&classes=&sum=&use=&date=&classUid=&remark=',
    })
  },
  editRemain: function (e) {
    let id = e.target.id;
    if (this.data.userType != 'admin' && this.data.userType != 'editor' && this.data.userType != 'comptAdmin' && this.data.userType!='claddingAdmin') {
      wx.showToast({
        title: '无访问权限',
        image: '/images/about.png'
      })
    ...
    wx.navigateTo({
      url: '/pages/detail/detail?id=' + this.data.displayClasses[id].claddingUid + '&db=compt&classes=' + this.data.displayClasses[id].classes + '&sum=' + this.data.displayClasses[id].remain + "&use=&date=&classUid=&remark=",
    })
  },
  openHistory: function () {
    wx.navigateTo({
      url: '/pages/history/history?db=compt',
    })
  },
